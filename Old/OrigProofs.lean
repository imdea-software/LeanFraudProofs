...
theorem goodProposersWin
  ...
  | leaf _v =>
    ...
    simp
    intro Hproposed
    simp [propTree, medTrees,ABTree.getI,ABTree.map,ABTree.getI'] at Hproposed
    simpa [simpGoodGen, propTree, ABTree.map, condWProp, BTree.map, treeCompArbGame]
    -- assumption
  | node al ar IndL IndR =>
    intro chooser
    simp [treeArbitrationGame, simpGoodGen,propTree, BTree.map, condWProp,ABTree.getI,ABTree.map, treeCompArbGame]
    cases chooser with
    | node cfun cl cr =>
      simp [ABTree.map]
      cases HFun : cfun
        ...
        | some nextSt =>
          cases  nextSt with
          | Now =>
            simp [medTrees, propTree, ABTree.map,ABTree.getI']
            -- unfold treeCompArbGame
            -- simp [BTree.map, simpGoodGen, propTree, ABTree.map]
          | Continue subTree =>
            cases subTree with
            | Left =>
              simp
              ...
              simp [treeArbitrationGame, condWProp] at indApp
              assumption
            | Right =>
              simp
...
theorem goodChoosersWin
  ...
  | leaf _v =>
    ...
    | leaf kh =>
      cases kh with
      | none => simp [ABTree.map, treeCompArbGame]
      | some hP =>
        simp [medTrees, propTree,ABTree.map,ABTree.getI,ABTree.getI'] at badTop
        simp [ABTree.map, treeCompArbGame, condWProp]
        intro _eqHash -- Should it be the hash of the element?
        assumption
  ...
  | node bl br IL IR =>
    ...
    | node mProp proposerLeft proposerRight =>
      ...
      | some props =>
        simp [treeCompArbGame, propTree, simpChooser, ChooserStr]
